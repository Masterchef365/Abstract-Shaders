...
vec3 hsv2rgb(vec3 c) {
	vec4 K = vec4((cos(u_time) + 1.) / 2., 2.0 / 1.752, 1.0 / 4.136, 4.056);
...
float vrand(vec2 pos) {
    return rand(pos.y + rand2(pos.x));
}

const int REFINEMENTS = 5;
float closest_t(QuadraticBezier q, vec2 pos) {
    float fuzz = vrand(floor(pos * 100. * abs(cos(u_time / 10.))) / 5.) / 45.636;
    float a = 0. - fuzz;
    float b = 1. + fuzz;
...
void main() {
    ...
    vec3 color = vec3(hsv2rgb(vec3(dist + 0.072, 0.736, 1.072)));
