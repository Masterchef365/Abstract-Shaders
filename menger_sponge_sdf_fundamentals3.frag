...
float menger_sponge_sdf(vec3 p) {
    float dist;
    if (inside_cube(p, dist, vec3(1))) {
        dist = menger_internals((p + 1.) / 2.);
    }
    return dist;
}
...
void main() {
    ...
    //p.z = fract(u_time / 10.) * 2. - 1.;
    p *= 2.;
    
    float dist = menger_sponge_sdf(p);
    //dist = max(dist, sqrt(1.) - distance(p, vec3(0.)));
    
    vec3 color = vec3(sgncolor(dist));
    color = vec3(dist < 0.);
