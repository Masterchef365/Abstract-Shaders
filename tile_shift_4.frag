...
vec3 pixel(vec2 coord) {
    ...
    float mask = rand_tiles(st, vec3(3303.528,1973.760,5234.000));
    ...
    if (mask < 0.164) {
        ...
        color = vec3(k) * vec3(0.935,1.000,0.908);
    } else if (mask < 0.440) {
        float k = rand_tiles(
            st + b.yx * base_height_ratio * anim_b, 
            vec3(5234.000,3578.864,4535.951)
        );
        color = vec3(k) * vec3(0.830,0.990,1.000);
    } else if (mask < 0.600) {
       float k = rand_tiles(
            st + c.yx * base_height_ratio * anim_c, 
            vec3(5234.000,3578.864,4535.951)
        );
        color = vec3(k) * vec3(0.806,1.000,0.888);
    } else {
        float k = rand_tiles(
            st, 
            vec3(5234.000,3578.864,4535.951)
