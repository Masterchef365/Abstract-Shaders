...
vec3 pixel(vec2 coord) {
    ...
    //vec4 psi = bloch(0., rand(st) * PI);
    psi = polarizer(st - vec2(0), -0.928, psi);
    psi = polarizer(st - vec2(-0.260,-0.040), PI/2., psi);
    psi = polarizer(st - vec2(0), -2.288, psi);

    vec3 color = vec3(prob(psi, spin_z_pos) * 1.);
