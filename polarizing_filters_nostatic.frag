...
float rand(vec2 co){
    return fract(sin(dot(co + u_time/10000., vec2(12.9898, 78.233))) * 43758.5453);
}
...
vec3 pixel(vec2 coord) {
    ...
    psi = polarizer(st - vec2(0), 0., psi);

    // Apply a rotating filter in the middle
    psi = polarizer(st - vec2(0.2,-0.2), u_time/7., psi);

    // Apply a vertical filter
    psi = polarizer(st - vec2(0.), PI/2., psi);

    // Measure probability with respect to the z axis 
    // (and boost the gain a bit so it's easier to see)
    vec3 color = vec3(prob(psi, spin_z_pos)*3.);
