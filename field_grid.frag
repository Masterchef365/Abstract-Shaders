...
vec3 pixel(vec2 coord) {
    ...
    vec3 color = sgncolor(v * 0.174);
    if (fract(abs(v) * 4.) < 0.058) color = vec3(1);

    vec2 e = q1 * normalize(q1_diff) / (q1d * q1d);
    e += q2 * normalize(q2_diff) / (q2d * q2d);
    
    //if (abs(fract(atan(e.y, e.x) * 8. / pi) - .5) < 0.052) color = vec3(0.125,1.000,0.432);
 	//if (abs(fract(atan(e.y, e.x) * 8. / pi) - .5) < 0.052) color = vec3(0.125,1.000,0.432);
 
    
    //color += vec3(abs(proj(e, vec2(0.230,0.860))) < 0.032);
    //color += vec3(abs(cross_mag(e, vec2(-0.190,0.710))) < 0.025);
    //color = vec3(abs(e), 0) / 20.;
    //color += vec3(abs(cross_mag(normalize(e), vec2(-0.950,0.000))) < 0.012);
    //color += vec3(fract(length(e)) < 0.108);
    
	color = vec3(any(lessThan(fract(e / 1.328 + u_time / 10.), vec2(0.05))));
