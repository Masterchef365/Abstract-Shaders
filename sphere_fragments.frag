vec4 wonk;
#define gl_FragColor wonk
#define uniform highp
...
float rand(vec3 co){
    return fract(sin(dot(co, vec3(12.9898, 78.233, 42.342))) * 43758.5453);
}
...
vec3 f_color(Sdf sdf, vec3 end) {
    if (sdf.mat == MAT_BACKGND) {
		...
		return vec3(fract(dot(end, vec3(0.420,-0.260,1.000)) * 20.));
