...
vec3 pixel(vec2 coord) {
    ...
    vec2 off = vec2(0.);
    off += float(inc_a) * a.yx * lin_slopestep(u_time, 2.) * base_height_ratio;
    off += float(inc_b) * b.yx * lin_slopestep(u_time + 1., 2.) * base_height_ratio;
    
    ivec3 slid_tiles = tri_tile(st + off);
    
    float k = seed_rand(vec3(slid_tiles), vec3(5234.000,3578.864,4535.951));
    vec3 color = mix(vec3(0.225,0.777,inc_a), vec3(inc_b), k);
