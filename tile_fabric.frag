...
float wick(vec2 pos) {
    ...
    for (int i = 0; i < 149; i++) { 
        ...
        q = q + dot(q, vec2(0.430,-0.650));
        if (abs(q.x) < 1.98 && abs(q.y) < 1.98) c += 0.007;
    }
    return c;
}

vec2 tri_quantize(vec2 p, float divs, out ivec3 gp) {
    const float pi = 3.141592;
    const float a = 0.;
...
vec3 pixel(vec2 p) {
    ...
    ivec3 g;
    vec2 quant = tri_quantize(st, 3., g);
    mat2 r = rot2d(quant.x + quant.y);
    vec3 q = fract(vec3(g) / 2.);
    float t = mod(q.x + q.y - q.z, 3.);
    
	const vec3 blue = vec3(214, 2, 112) / 255.0;
    const vec3 white = vec3(155, 79, 150) / 255.0;
    const vec3 pink = vec3(28.550,83.912,168.000) / 255.0;
